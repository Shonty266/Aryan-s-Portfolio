...
const Projects = () => {
  ...
  const [loading, setLoading] = useState(
    Array(data.length).fill(true) // Track loading state for each image
  );
  ...
  const handleImageLoad = (index) => {
    setLoading((prev) => {
      ...
      updatedLoading[index] = false; // Set the loading state of the image to false when loaded
        ...
        <div className="flex items-center justify-start pt-8 pb-2 gap-4 flex-wrap">
          <button
            ...
            className={`duration-300 font-semibold lg:text-md text-md py-1 px-4 rounded-md ${
              activeType === ""
                ? "bg-[var(--background-button)] text-[var(--text-skill-active)] shadow-black"
                : "bg-[var(--background-button-bg)] shadow-black "
            }`}
            style={{
        ...
        <div className="w-full flex justify-center items-center" id="projects">
          <div className="project-item grid grid-cols-1 lg:grid-cols-2 gap-10 pt-4 w-full">
            {projects.map((item, index) => (
              ...
              >
                {loading[index] && (
                  <div className="w-full lg:h-96 h-50 flex justify-center items-center bg-gray-200">
                    <p>Loading...</p>
                  </div>
                )}
                <img
                  src={item.image}
                  alt={item.name}
                  className={`w-full lg:h-96 h-50 object-cover rounded-lg shadow-md shadow-gray-400 ${
                ...
                <div className="flex flex-col justify-between px-4 pb-4 pt-4">
                  <div className="flex gap-4 items-center mt-2">
                    <div className="flex items-center pb-4">
                      <p className="font-bold lg:text-7xl text-4xl text-[#cfd8dc]">
                        0{item.id}
                      </p>
                    </div>
                    <div className="flex flex-col flex-wrap">
                      ...
                      <p className="lg:text-md text-md text-[#a0b1ba] font-semibold lg:mt-2">
                        {item.projectType}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-row pb-4 lg:items-center items-end">
                    ...
                    <div className="tooltip-container relative inline-block pl-2 mt-4">
                      ...
                      <p className="tooltip-text bottom-[-60%] lg:bottom-[-50%]">
                        {item.location}
                      </p>
                    </div>
                  </div>
